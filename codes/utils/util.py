...
def gasser_muller_kernel(x, h):
    """
    Compute the Gasser-Müller kernel function.

    Parameters:
        x (float): The point at which to evaluate the kernel function.
        h (float): The bandwidth of the kernel.

    Returns:
        float: The value of the Gasser-Müller kernel function at x.
    """
    return norm.pdf(x / h) - (x / h) * norm.cdf(x / h)
...
def anisotropic_gaussian_kernel_matlab(l, sig1, sig2, theta, tensor=False):
    ...
    # x_m,h,c = pos.shape
    # for i in range(x_m):
        # k_m = gasser_muller_kernel(pos[i][i][0], pos[i][i][1])
        # muller.append(k_m)
    # muller = np.array(muller)
    # print(muller)
    # sys.exit()
...
class BatchBlur_SV(nn.Module):
    ...
    def forward(self, input, kernel):
        ...
        if len(kernel.size()) == 2:
            ...
            # print(pad.size())
            cb, hw, _ =pad.size()
            kernel = kernel.view(1,-1,441).permute(0,2,1)
            _,_,k_1 = kernel.size()
            # print(kernel.size())
            # sys.exit()
            kernel = F.interpolate(kernel, scale_factor=hw/k_1, mode='nearest').permute(0, 2, 1)
            # kernel = kernel.flatten(2).expand(3,-1,-1)
            kernel = kernel.flatten(2).expand(3,-1,-1,-1)
            out_unf = (pad*kernel.contiguous().view(-1,kernel.size(2),kernel.size(3))).sum(2).unsqueeze(1)
            # sys.exit()
            out = F.fold(out_unf, (H, W), 1).view(B, C, H, W)
...
class SRMDPreprocessing(object):
    ...
    def __call__(self, hr_tensor, kernel=False):
        ...
        lr_np = tensor2img(lr)
        lr_bi = cv2.bilateralFilter(lr_np, -1, 10, 5)
        lr_bi = img2tensor(lr_bi)
        lr_bi = lr_bi.unsqueeze(0)
