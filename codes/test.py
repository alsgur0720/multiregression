...
for test_loader in test_loaders:
    ...
    for test_data in test_loader:
        ...
        LR_img = util.tensor2img(LR_img)  # uint8
        ...
        # print(visuals['KE'].shape)
        # sys.exit()
        # choose a kernel to visualize from SV kernels
        if len(visuals['KE'].shape) > 2:
            est_ker = util.tensor2img(visuals['KE'][5, :, :], np.float32)
        ...
        # if real_image:
        #     util.plot_kernel(est_ker, save_ker_path,kernel_preset[0])
        #     if args.save_kernel and est_ker_sv is not None:
        #         np.savez(save_ker_SV_path, sr_img=sr_img, est_ker_sv=est_ker_sv, gt_ker=kernel_preset[0])
        ...
        # print(gt_img_lr.shape)
        # save_lr_img_path = os.path.join(dataset_dir, img_name + '_ori_lr' + '.png')
        # save_gt_img_path = os.path.join(dataset_dir, img_name + '_ori_gt' + '.png')
        # util.save_img(sr_img_lr , save_lr_img_path)
        # util.save_img(gt_img_lr , save_gt_img_path)
        # sr_img_lr = cv2.Canny(sr_img_lr, 100, 255)    
        # gt_img_lr = cv2.Canny(gt_img_lr, 100, 255)
        # save_lr_img_path = os.path.join(dataset_dir, img_name + '_canny_lr' + '.png')
        # save_gt_img_path = os.path.join(dataset_dir, img_name + '_canny_gt' + '.png')
        # # print(sr_img_l)  
        # util.save_img(sr_img_lr , save_lr_img_path)
        # util.save_img(gt_img_lr , save_gt_img_path)
        # # print(save_lr_img_path)
        # # sys.exit()
        ...
        # if crop_border == 0:
        #     cropped_sr_img_lr_y = sr_img_lr
        #     cropped_gt_img_lr_y = gt_img_lr
        # else:
        #     cropped_sr_img_lr_y = sr_img_lr[crop_border:-crop_border, crop_border:-crop_border]
        #     cropped_gt_img_lr_y = gt_img_lr[crop_border:-crop_border, crop_border:-crop_border]
        ...
        if not real_image:
            ...
            #     gt_ker = est_ker
            # est_ker= est_ker.float().cpu().numpy().astype(np.float32)
