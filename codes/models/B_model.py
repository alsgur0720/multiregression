...
class B_Model(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            self.netG.train()
            ## param loss
            self.param_mse = nn.MSELoss().to(self.device)
            self.cri_pix_parametic = nn.L1Loss().to(self.device)
            # HR loss
    ...
    def optimize_parameters(self, step, scaler):
        ...
        with autocast():
            ...
            self.fake_SR, self.fake_K, est_param = self.netG(self.var_LN, self.real_K)
            b, _ = est_param.size()
            param_loss = self.param_mse(self.ker_map, est_param)
            # print('kermap:',self.ker_map, 'param:', param)
            l_all += param_loss
            
            param = est_param.detach().cpu().numpy()
            param[param < 0.01] = 0.01
            
            est_batch_kernel = torch.zeros((b, 21, 21)).to('cuda:0')
            for i in range(b):
                est_batch_kernel[i, :, :] =  util.anisotropic_gaussian_kernel_matlab(21, param[i,0], param[i,1], param[i,2], tensor=True)
            est_batch_kernel = torch.FloatTensor(est_batch_kernel).to('cuda:0')
            
            param_k = self.cri_pix_parametic(est_batch_kernel*1000, self.real_K*1000)
            self.log_dict['param_k'] = param_k.item()
            
            self.log_dict['param_loss'] = param_loss.item()
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            ### non sv mode
            # self.fake_SR, kernelorg, param = self.netG(self.var_LN, self.real_K)
            ...
            ## sv mode
            self.fake_SR, self.fake_K, self.param = self.netG(self.var_LN, self.real_K)
            b,c,p = self.param.size()
            param = self.param.detach()[0].float().cpu()
            # param_m = torch.zeros((b,c,21,21), device = 'cuda:0', requires_grad=True).clone()
            # b,p,_ = self.param.shape
            # self.fake_K
            k_0, k_1, k_2, k_3 =self.fake_K.size()
            fake_k = self.ada(self.fake_K.permute(0,2,3,1).view(k_0,k_2,k_3,k_1))
            # print(self.fake_K.size())

            # fake_k = self.fake_K.view(b,-1,21,21)
            
            
            param_m = torch.zeros((b,c,21,21), device = 'cuda:0', requires_grad=True).clone()
            for i in range(0, c):
                est_ker = util.anisotropic_gaussian_kernel_matlab(21, param[i][0], param[i][1], param[i][2])
                est_ker = torch.Tensor(est_ker)
                param_m[0, i, :,:] = est_ker
            # print(param_m.size())
            # sys.exit()
            ...
            if self.cal_lr_psnr:
                ...
                if self.real_H.shape[2] * self.real_H.shape[3] > 1:
                    # self.fake_LR = self.degradation_model(self.real_H, self.fake_K)
                    self.fake_LR = self.degradation_model(self.real_H, fake_k)
                # no HR
                else:
                    # self.fake_LR = self.degradation_model(self.fake_SR, self.fake_K)
                    self.fake_LR = self.degradation_model(self.fake_SR, fake_k)
    ...
    def get_current_visuals(self):
        ...
        # out_dict['KE'] = self.fake_K
        out_dict['KE'] = self.fake_K[0].detach().float().cpu()
        # out_dict['KE'] = self.fake_K.detach().float().cpu()
        out_dict['K'] = self.real_K.detach()[0].float().cpu()
        # out_dict['param'] = self.param.detach()[0].float().cpu()
        out_dict['Batch_SR'] = self.fake_SR.detach().float().cpu()  # Batch SR, for train
        out_dict['Batch_KE'] = self.fake_K  # Batch SR, for train
        return out_dict
...
class DegradationModel(nn.Module):
    def __init__(self, kernel_size=15, scale=4, sv_degradation=False):
